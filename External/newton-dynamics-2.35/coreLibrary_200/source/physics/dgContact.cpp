...
//	m_collisionContinueCollisionEnable = true;
  m_flags = m_collisionEnable__ | m_friction0Enable__ | m_friction1Enable__
      | m_collisionContinueCollisionEnable__;
}
    ...
    dgInt32& frictionIndex)
{
  ...
  params.m_jointAccel[normalIndex] = GetMax(dgFloat32(-4.0f),
      relVelocErr + penetrationVeloc) * params.m_invTimestep;
    ...
    if (!params.m_accelIsMotor[k])
    {
      ...
      if (params.m_normalForceIndex[k] < 0)
      {
        ...
        if (params.m_penetration[k] > dgFloat32(1.0e-2f))
        {
          ...
          penetrationVeloc = -(params.m_penetration[k]
              * params.m_penetrationStiffness[k]);
        }
