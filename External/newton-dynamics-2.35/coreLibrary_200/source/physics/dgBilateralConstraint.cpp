...
{
  return (DG_JOINT_STIFFNESS_RANGE - m_stiffness)
      / (DG_JOINT_STIFFNESS_RANGE - dgFloat32(1.0f));
}

void dgBilateralConstraint::SetStiffness(dgFloat32 stiffness)
{
  stiffness = ClampValue(stiffness, dgFloat32(0.0f), dgFloat32(1.0f));
  m_stiffness = DG_JOINT_STIFFNESS_RANGE
      - stiffness * (DG_JOINT_STIFFNESS_RANGE - dgFloat32(1.0f));
}
    ...
    const dgVector& pinDirection0, const dgVector& pinDirection1)
{
  ...
  m_localMatrix0.m_right =
      m_localMatrix0.m_right.Scale(
          dgFloat32(
              1.0f) / dgSqrt (m_localMatrix0.m_right % m_localMatrix0.m_right));
    ...
    const dgJointAccelerationDecriptor& params)
{
  ...
  for (dgInt32 k = 0; k < params.m_rowsCount; k++)
  {
    if (m_rowIsMotor[k])
    {
      params.m_coordenateAccel[k] = m_motorAcceleration[k]
          + params.m_externAccelaration[k];
    }
    else
    {
      dgFloat32 num;
      ...
      relPosit = params.m_penetration[k]
          - vRel * dt * params.m_firstPassCoefFlag;
