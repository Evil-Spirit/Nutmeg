...
function GetDisplayName(Parentfolder: IShellFolder; PIDL: PItemIDList; Flags: DWORD; ShowExt: TacShowExtension): string;
...
{$ENDIF}
    ...
    STRRET_WSTR : if Assigned(StrRet.pOleStr) then Result := StrRet.pOleStr else Result := '';
  end;
...
{$IFDEF TNTUNICODE}
function StrRetToStringW(PIDL: PItemIDList; StrRet: TStrRet; Flag: string = ''): WideString;
var
  P: PChar;
begin
  case StrRet.uType of
...
var
  StrRet: TStrRet;
  NewFilename, ExtOld, ExtNew:string;
begin
  Result := '';
...
{$ELSE}
      RootNode.Text := GetDisplayName(DesktopShellFolder, TacShellFolder(RootNode.Data).AbsoluteID, SHGDN_NORMAL, FShowExtension); // v6.63
{$ENDIF}
    ...
    try
      ...
      while EnumList.Next(1, ID, NumIDs) = S_OK do begin
        ...
        if AFolder.IsFolder or (w = 0) then CanAdd := True else begin
          CanAdd := False;
          s := AFolder.DisplayName(seShow);// AFolder.PathName;
