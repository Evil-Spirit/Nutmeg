...
uses sGraphUtils, acAlphaImageList, CommCtrl, acntUtils, acPNG, sButton, sAlphaGraph, math {$IFDEF DEVEX}, acLFPainter{$ENDIF} { // for projects which uses the DEVEX key};
...
begin
  ...
  if Assigned(Glyph) and not Glyph.Empty then begin
    if Glyph.PixelFormat = pfDevice then begin
